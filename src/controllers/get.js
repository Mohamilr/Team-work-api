    ...
    try {
      ...
      if (!getArticles.rowCount || !getGifs.rowCount) {
        return jsonResponse(
          ...
          'error',
          400,
          'sorry, there are no articles or gifs available in the database'
        );
      }
  ...
  async getSingleArticle(req, res) {
    ...
    try {
      ...
      const comment = await pool.query(
        'SELECT commentid, comment, authorid FROM article_comments WHERE articleId=$1',
        [id]
      );
  ...
  async getSingleGif(req, res) {
    ...
    try {
      ...
      const comments = await pool.query(
        `SELECT commentid, comment, authorid FROM gif_comments WHERE gifId=${id}`
      );
