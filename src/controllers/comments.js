...
const comments = {
  async articleComment(req, res) {
    ...
    try {
      ...
      sendQuery = await pool.query(
        `INSERT INTO article_comments (comment, createdOn, authorId, articleId)
                VALUES($1, $2, $3, $4) RETURNING *`,
        [comment, new Date().toLocaleString(), authorId, id]
      );
  ...
  async gifComment(req, res) {
    ...
    try {
      ...
      sendQuery = await pool.query(
        'INSERT INTO gif_comments (comment, createdon, authorid, gifid) VALUES($1, $2, $3, $4) RETURNING *',
        [comment, new Date().toLocaleString(), authorId, id]
      );
